...
class CreateTableTxDisputesAll extends AbstractMigration
{
  public function up()
  {
    ...
    $this->addBigInt($disputesTable, 'deleted', ['length' => 20, 'null' => true, 'default' => null, 'comment' => "unix timestamp of when the dispute record was deleted, null if it hasn't been"]);
    $disputesTable->create();

    $this->execute('CREATE VIEW tx_disputes AS SELECT id, xid, uid, agentUid, reason, status FROM tx_disputes_all WHERE deleted IS NULL');
    $this->execute('CREATE VIEW tx_disputes_deleted AS SELECT id, xid, uid, agentUid, reason, status, deleted FROM tx_disputes_all WHERE deleted IS NOT NULL');
  }
