...
class CreateTableRTxHdrs extends AbstractMigration
{
  public function up()
  {
    ...
    /* $hdrTable->addForeignKey('boxId', 'r_boxes', 'id', ['delete' => 'RESTRICT', 'update' => 'CASCADE']); */
    /* $hdrTable->addForeignKey('reversesXid', 'r_tx_hdrs', 'xid', ['delete' => 'RESTRICT', 'update' => 'CASCADE']); */

    $hdrTable->create();

    $this->execute('CREATE VIEW r_tx_hdrs AS SELECT xid, type, goods, actorId, actorAgentId, flags, channel, boxId, risk, risks, reversesXid, created FROM all_tx_hdrs WHERE deleted IS NULL');
    $this->execute('CREATE VIEW x_tx_hdrs AS SELECT xid, type, goods, actorId, actorAgentId, flags, channel, boxId, risk, risks, reversesXid, created, deleted FROM all_tx_hdrs WHERE deleted IS NOT NULL');
  }
