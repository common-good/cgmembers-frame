...
class CreateTableRTxHdrs extends AbstractMigration
{
  public function up()
  {
    ...
    $this->addTinyInt($hdrTable, 'type', ['length' => 4, 'null' => false, 'default' => '0', 'comment' => 'transaction type (transfer, rebate, etc.)']);
    $this->addTinyInt($hdrTable, 'goods', ['length' => 4, 'null' => false, 'default' => '0', 'comment' => 'is this transfer an exchange for real goods and services?']);
    $this->addBigInt($hdrTable, 'actor', ['length' => 20, 'null' => false, 'comment' => "user id of the transaction's initiator"]);
    $this->addBigInt($hdrTable, 'actorAgent', ['length' => 20, 'null' => true, 'comment' => 'user id of the agent for the initiator (who actually initiated this transaction)']);
    ...
    $hdrTable->addIndex(['actor']);
    $hdrTable->addIndex(['created']);
    $hdrTable->addIndex(['reverses'], ['unique' => true]);

    /* $hdrTable->addForeignKey('actor', 'users', 'uid', ['delete' => 'RESTRICT', 'update' => 'CASCADE']); */
    /* $hdrTable->addForeignKey('actorAgent', 'users', 'uid', ['delete' => 'RESTRICT', 'update' => 'CASCADE']); */
    ...
    $this->execute('CREATE VIEW r_tx_hdrs AS SELECT xid, type, goods, actor, actorAgent, flags, channel, box, risk, risks, reverses, created FROM all_tx_hdrs WHERE deleted IS NULL');
    $this->execute('CREATE VIEW x_tx_hdrs AS SELECT xid, type, goods, actor, actorAgent, flags, channel, box, risk, risks, reverses, created, deleted FROM all_tx_hdrs WHERE deleted IS NOT NULL');
  }
