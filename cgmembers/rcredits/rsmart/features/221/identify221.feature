...
Scenario: Device has no identifier yet for an individual
  ...
  Then we respond with:
  | ok | name    | descriptions | canMgr                                    | bad | device | default | name    | time |*
  | 1  | Abe One |              | charge,undo,refund,r4usd,usd4r,buy,manage |     | ?      | NEWZZA  | Abe One | %now |
  
Scenario: Device has no identifier yet for trial company
  When agent "" asks device "" to sign in ".ZZH,ccH"
  Then we respond with:
  | ok | name    | descriptions | canMgr              | bad | device | default | company | time |*
  | 1  | Hill Co | hills        | charge,r4usd,manage |     | ?      | NEWZZH  | Hill Co | %now |
