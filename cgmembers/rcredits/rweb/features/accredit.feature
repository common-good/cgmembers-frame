...
Scenario: A member company gives a customer credit again before it runs out
  When member "C:B" confirms "co/accredit" with:
  | customer      | amount |*
  | a@example.com |      3 |
  
  And member "C:B" confirms "co/accredit" with:
  | customer      | amount |*
  | a@example.com |      2 |
  Then these "tx_rules":
  | id        | 1 |**
  ...
  And we message "store grant|to redeem|want a card|finish get card" to member ".ZZA" with subs:
  | co      | amount | topic           |*
  | Cor Pub | $2     | You got credit! |
  And we message "you gave credit" to member ".ZZC" with subs:
  | nm      | amount | topic                      |*
  | Abe One | $2     | You gave a customer credit |
