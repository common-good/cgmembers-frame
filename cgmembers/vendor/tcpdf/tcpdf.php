...
class TCPDF {
	...
	public function Output($name='doc.pdf', $dest='I') {
		...
		switch($dest) {
          ...
          header_remove('Content-Security-Policy'); // cg 2018-04-01
...
//		while (list($key, $val) = each($prop)) {
		foreach ($prop as $key => $val) { // CGF 2018-02 each($prop)) {
...
//					while (list($id, $name) = each($attr_array[1])) {
					foreach ($attr_array[1] as $id => $name) { // CGF 2018-02 each($attr_array[1])) {
...
//						while (list($id, $name) = each($style_array[1])) {
						foreach ($style_array[1] as $id => $name) { // CGF 2018-02  each($style_array[1])) {
